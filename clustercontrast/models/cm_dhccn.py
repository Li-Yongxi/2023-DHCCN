...
class CM_Ori(autograd.Function):
    ...
    @staticmethod
    def forward(ctx, inputs, targets, features, momentum, prob=None):
        ctx.features = features
        ctx.momentum = momentum
        ctx.save_for_backward(inputs, targets, prob)
        outputs = inputs.mm(ctx.features.t())
    ...
    @staticmethod
    def backward(ctx, grad_outputs):
        inputs, targets, prob = ctx.saved_tensors
        grad_inputs = None
        if ctx.needs_input_grad[0]:
        ...
        for x, y in zip(inputs, targets):
            ctx.features[y] = ctx.momentum * ctx.features[y] + (1. - ctx.momentum) * x * 0.1
            ctx.features[y] /= ctx.features[y].norm()
        # else:
        #     for x, y, p in zip(inputs, targets, prob.cuda()):
        #         ctx.features[y] = ctx.momentum * ctx.features[y] + (1. - ctx.momentum) * p * x
        #         ctx.features[y] /= ctx.features[y].norm()

        return grad_inputs, None, None, None, None

def cm(inputs, indexes, features, momentum=0.5, prob=None):
    # return CM.apply(inputs, indexes, features, torch.Tensor([momentum]).to(inputs.device))
    return CM_Ori.apply(inputs, indexes, features, torch.Tensor([momentum]).to(inputs.device), prob)
...
class ClusterMemory(nn.Module, ABC):
    def __init__(self, num_features, num_samples, temp=0.05, momentum=0.2, use_hard=False):
        ...
        self.register_buffer('prob', torch.zeros(num_samples, num_features))


    def cal_loss(self, inputs, targets, features):
        output = cm(inputs, targets, features, self.momentum, self.prob)
        output /= self.temp
    ...
    def forward(self, inputs, targets, epoch=0):
        ...
        log_mean = ((self.global_p1_features.softmax(dim=-1) + self.global_p2_features.softmax(dim=-1)) / 2).log()
        loss_js = (F.kl_div(log_mean, self.global_p1_features.softmax(dim=-1), reduction='sum') + F.kl_div(log_mean, self.global_p2_features.softmax(dim=-1), reduction='sum')) / 2
